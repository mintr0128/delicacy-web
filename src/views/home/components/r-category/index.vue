<template>
  <div @mouseleave="hideProductCate">
    <div class="r-category-components text-sm">
      <ul>
        <li @click="goSearch(item)" v-for="(item, index) in menuListData" :class="userSelectId === item.id ? 'active' : ''"
          :key="index" @mouseenter="showProductCate(item.id)">
          <div class="left-title">
            <i class="iconfont" :class="item.iconName"></i>
            <div class="cate-title">{{ item.name }}</div>
            <div class="cate-subtitle" v-if="item.children && item.children.length > 0">
              <span class="ellipsis" style="display: block;width: 70px" v-for="(subitem, subindex) in item.children"
...
export default {
  ...
  setup() {
    ...
    return { menuListData, showProductCate, hideProductCate, showProduct, userSelectCateData, userSelectId, goSearch }
  }
}
</script>
